...
public class DartUI extends SimpleTileUI {

	private static final Color defaultBackgroundColor = Colours.DARK_PURPLE; 
	...
	public DartUI(Vertex[] vertices, AffineTransform t) {
...
//		arc1.setAngleStart(360 - arc1.getAngleStart() - 72); // fix for reflection problem!
		...
		arc2.setArcByCenter(vertices[2].x, vertices[2].y,
						PenroseTile.PHI - 1.0, 0, 216, Arc2D.OPEN);
		// JDK 1.2.2
...
//		arc2.setAngleStart(360 - arc2.getAngleStart() - 216); // fix for reflection problem!

		ammannBars = new GeneralPath();
		org.tiling.alhambra.geom.Point2D point = Tools2D.pointOnLine(vertices[1], vertices[2], 0.25);
		ammannBars.moveTo((float) point.x, (float) point.y);
		point = Tools2D.pointOnLine(vertices[1], vertices[0], 1 / (2 * (1 + PenroseTile.PHI)));
		ammannBars.lineTo((float) point.x, (float) point.y);
		point = Tools2D.pointOnLine(vertices[3], vertices[0], 1 / (2 * (1 + PenroseTile.PHI)));

		ammannBars.lineTo((float) point.x, (float) point.y);
		point = Tools2D.pointOnLine(vertices[3], vertices[2], 0.25);
		ammannBars.lineTo((float) point.x, (float) point.y);
	}
